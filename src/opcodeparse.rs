...
0xE000 => match opcode & 0x00FF {
    0x9E => {
        ...
        if emu_state.keys[key as usize] != 0 {
            emu_state.pc += 2;
        }
    }
...
0xF000 => match opcode & 0x00FF {
    ...
    0x1E => {
        // ADD I, Vx - Set I = I + Vx
        let regdex = ((opcode & 0x0F00) >> 8) as usize;
        emu_state.ir = emu_state.ir + (emu_state.gpr[regdex] as u16);
    }
    
    0x29 => {
        // LD F, Vx - Set I = location of sprite for digit Vx
        let regdex = ((opcode & 0x0F00) >> 8) as usize;
        let digit = emu_state.gpr[regdex];
